...
class CreateFoodFragment : BaseFragment(), OnClickListener, OnItemSelectedListener {

    private var onActionBarListener: OnActionBarListener? = null
    private val viewModel: CreateFoodViewModel by viewModel()
    ...
    override fun setUpView() {
        ...
        toolbarCreateFood.toolbar.textViewToolbarTitle.text = getString(R.string.text_create_food)
        ...
        arguments?.getParcelableArrayList<Category>(EXTRA_CATEGORIES)?.let {
    ...
    override fun registerLiveData() {
        ...
        viewModel.onCreateFoodEvent.observe(this, Observer {
            context?.showAlertDialogBasic(getString(R.string.text_create_food_success), isShowCancelButton = false) {
            ...
            R.id.buttonCreate -> {
                if (validateForm()) {
                    viewModel.createFood(
                        categoryId,
                        imagePart.toRequestBodyImageType(),
    ...
    private fun validateForm(): Boolean {
        if (imagePart.isEmpty()) {
            context?.showToast(getString(R.string.text_no_image_error))
            return false
        }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            Constant.REQUEST_IMAGE_CAPTURE -> if (resultCode == Activity.RESULT_OK) cropImage(
                getCacheImagePath(
                    fileName
                ), isBitmapMaxSize = true
            )
            ...
            UCrop.REQUEST_CROP -> if (resultCode == Activity.RESULT_OK) handleUCropResult(data)
    ...
    companion object {
        ...
        fun newInstance(categories: ArrayList<Category>) = CreateFoodFragment().apply {
            arguments = Bundle().apply { putParcelableArrayList(EXTRA_CATEGORIES, categories) }
        }
    }
}
